...
{ // Frodo-KEM's key generation
  ...
  //          secret key sk (CRYPTO_BYTES + BYTES_SEED_A + (PARAMS_LOGQ*PARAMS_N*PARAMS_NBAR)/8 + 2*PARAMS_N*PARAMS_NBAR bytes)
    ...
    frodo_sample_n(B, PARAMS_N*PARAMS_NBAR, randomness + CRYPTO_BYTES, CRYPTO_BYTES, 2);
...
{ // Frodo-KEM's key encapsulation
    ...
    frodo_sample_n(B, PARAMS_N*PARAMS_NBAR, G, CRYPTO_BYTES, 5);
    frodo_mul_add_sa_plus_e(B, Sp, temp);
    frodo_pack(ct, (PARAMS_LOGQ*PARAMS_N*PARAMS_NBAR)/8, B, PARAMS_N*PARAMS_NBAR, PARAMS_LOGQ);
...
{ // Frodo-KEM's key decapsulation
    ...
    uint8_t temp[CRYPTO_CIPHERTEXTBYTES+CRYPTO_BYTES], G[3*CRYPTO_BYTES];
    const uint8_t *seed_A = sk+CRYPTO_BYTES;
    uint16_t Sp[PARAMS_N*PARAMS_NBAR] = {0};
    ...
    frodo_sample_n(Epp, PARAMS_NBAR*PARAMS_NBAR, G, CRYPTO_BYTES, 6);
    frodo_unpack(BBp, sk+CRYPTO_BYTES+BYTES_SEED_A, CRYPTO_PUBLICKEYBYTES - BYTES_SEED_A);

    frodo_mul_add_sb_plus_e(W, BBp, Sp, Epp);

    //Generate Ep, and compute BBp = Sp*A + Ep. Generate A on-the-fly
    ...
    {  // Load (KK || d) to do ss = F(ct||KK||d)
        memcpy(&temp[CRYPTO_CIPHERTEXTBYTES-CRYPTO_BYTES], &G[CRYPTO_BYTES], 2*CRYPTO_BYTES);
    }
    else
    {  // Load (s || d) to do ss = F(ct||s||d)
