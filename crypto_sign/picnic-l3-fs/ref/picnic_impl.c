...
void LowMCEnc(const uint32_t* plaintext, uint32_t* output, uint32_t* key, paramset_t* params)
{
    ...
    matrix_mul(key, KMatrix(0), roundKey, params);
    xor_array(output, roundKey, output, params->stateSizeWords);

    for (uint32_t r = 1; r <= params->numRounds; r++) {
        matrix_mul(key, KMatrix(r), roundKey, params);
        substitution(output, params);
        matrix_mul(output, LMatrix(r - 1), output, params);
        xor_array(output, RConstant(r - 1), output, params->stateSizeWords);
        xor_array(output, roundKey, output, params->stateSizeWords);
    }
                      ...
                      uint32_t tapeLengthBytes, paramset_t* params)
{
    ...
    shake256_absorb(s, (uint8_t*)&outputBytesLE, sizeof(uint16_t));
    shake256_squeezeblocks(tape, MAX(1,tapeLengthBytes/SHAKE256_RATE), s);
            ...
            uint8_t* hash, paramset_t* params)
{
    ...
    uint64_t s[25] = {0};
    shake256_absorb(s, &HASH_PREFIX_4, sizeof(HASH_PREFIX_4));
    shake256_absorb(s, seed, params->seedSizeBytes);
    shake256_squeezeblocks(hash, 1, s);
    ...
    memset(s, 0, sizeof(s));
    shake256_absorb(s, &HASH_PREFIX_0, sizeof(HASH_PREFIX_0));
    shake256_absorb(s, (uint8_t*)view.inputShare, params->stateSizeBytes);
    shake256_absorb(s, (uint8_t*)view.communicatedBits, params->andSizeBytes);
    shake256_absorb(s, (uint8_t*)view.outputShare, params->stateSizeBytes);
...
void G(uint8_t viewNumber, const uint8_t* seed, view_t* view, uint8_t* output, paramset_t* params)
{
    ...
    uint64_t s[25] = {0};
    shake256_absorb(s, &HASH_PREFIX_5, sizeof(HASH_PREFIX_5));
    shake256_absorb(s, seed, params->seedSizeBytes);
    shake256_squeezeblocks(output, 1, s);
    ...
    memset(s, 0, sizeof(s));
    shake256_absorb(s, output, params->digestSizeBytes);
    //HashInit(&ctx, params, HASH_PREFIX_NONE);
    ...
    shake256_absorb(s, view->communicatedBits, params->andSizeBytes);
    ...
    shake256_absorb(s, (uint8_t*)&outputBytesLE, sizeof(uint16_t));
    shake256_squeezeblocks(output, MAX(1,outputBytes/SHAKE256_RATE), s);
}
        ...
        g_commitments_t* gs, paramset_t* params)
{
    ...
    /* Hash input data */
    uint64_t s[25] = {0};
    shake256_absorb(s, &HASH_PREFIX_1, sizeof(HASH_PREFIX_1));
    ...
    for (uint32_t i = 0; i < params->numZKBRounds; i++) {
        for (int j = 0; j < 3; j++) {
            ...
            shake256_absorb(s, (uint8_t*)VIEW_OUTPUTS(i, j), params->stateSizeBytes);
        }
    }
    ...
    for (uint32_t i = 0; i < params->numZKBRounds; i++) {
        for (int j = 0; j < 3; j++) {
            ...
            shake256_absorb(s, as[i].hashes[j], params->digestSizeBytes);
        }
    }
    ...
    if (params->transform == TRANSFORM_UR) {
        for (uint32_t i = 0; i < params->numZKBRounds; i++) {
            for (int j = 0; j < 3; j++) {
                ...
                shake256_absorb(s, gs[i].G[j], view3UnruhLength);
    ...
    shake256_absorb(s, (uint8_t*)circuitOutput, params->stateSizeBytes);
    shake256_absorb(s, (uint8_t*)plaintext, params->stateSizeBytes);
    shake256_absorb(s, message, messageByteLength);
    ...
    while (1) {
        ...
        memset(s, 0, sizeof(s));
        shake256_absorb(s, &HASH_PREFIX_1, sizeof(HASH_PREFIX_1));
        shake256_absorb(s, hash, params->digestSizeBytes);
        shake256_squeezeblocks(hash, 1, s);
        //HashInit(&ctx, params, HASH_PREFIX_1);
        //HashUpdate(&ctx, hash, params->digestSizeBytes);
        //HashFinal(&ctx);
                      ...
                      const uint32_t* plaintext, paramset_t* params, uint8_t challenge)
{
    ...
    mpc_matrix_mul(keyShares, KMatrix(0), roundKey, params, 2);
    mpc_xor(state, roundKey, params->stateSizeWords, 2);

    for (uint32_t r = 1; r <= params->numRounds; ++r) {
        mpc_matrix_mul(keyShares, KMatrix(r), roundKey, params, 2);
        mpc_substitution_verify(state, tapes, view1, view2, params);
        mpc_matrix_mul(state, LMatrix(r - 1), state, params, 2);
        mpc_xor_constant_verify(state, RConstant(r - 1), params->stateSizeWords, challenge);
        mpc_xor(state, roundKey, params->stateSizeWords, 2);
    }
               ...
               const uint32_t* plaintext, uint32_t* slab, paramset_t* params)
{
    ...
    mpc_matrix_mul(keyShares, KMatrix(0), roundKey, params, 3);
    mpc_xor(state, roundKey, params->stateSizeWords, 3);

    for (uint32_t r = 1; r <= params->numRounds; r++) {
        mpc_matrix_mul(keyShares, KMatrix(r), roundKey, params, 3);
        mpc_substitution(state, tapes, views, params);
        mpc_matrix_mul(state, LMatrix(r - 1), state, params, 3);
        mpc_xor_constant(state, RConstant(r - 1), params->stateSizeWords);
        mpc_xor(state, roundKey, params->stateSizeWords, 3);
    }
                      ...
                      publicKey, uint32_t* plaintext, const uint8_t* message, size_t messageByteLength, paramset_t* params)
{
    ...
    uint64_t s[25] = {0};
    shake256_absorb(s, (uint8_t*)privateKey, params->stateSizeBytes);
    shake256_absorb(s, message, messageByteLength);
    shake256_absorb(s, (uint8_t*)publicKey, params->stateSizeBytes);
    shake256_absorb(s, (uint8_t*)plaintext, params->stateSizeBytes);
    //HashInit(&ctx, params, HASH_PREFIX_NONE);
    ...
    //HashFinal(&ctx);
    shake256_absorb(s, ((uint8_t*)&stateSizeBitsLE), sizeof(uint16_t));

    //HashSqueeze(&ctx, getSeed(allSeeds, 0, 0), params->seedSizeBytes * 3 * params->numZKBRounds);
    shake256_squeezeblocks(getSeed(allSeeds, 0, 0), MAX(1,(params->seedSizeBytes*3*params->numZKBRounds)/SHAKE256_RATE), s);
