...
int get_param_set(picnic_params_t picnicParams, paramset_t* paramset)
{
    ...
    */
    ...
    paramset->transform = TRANSFORM_FS; //get_transform(picnicParams);
...
int picnic_write_private_key(const picnic_privatekey_t* key, uint8_t* buf, size_t buflen)
{
    ...
    if (buflen < bytesRequired) {
        fprintf(stderr, "%s: buffer provided has %lu bytes, but %lu are required.\n", __func__, (uint32_t)buflen, (uint32_t)bytesRequired);
        return -1;
    }
...
void print_signature(const uint8_t* sigBytes, size_t sigBytesLen, picnic_params_t picnic_params )
{
    ...
    for (size_t i = 0; i < params.numZKBRounds; i++) {
        ...
        snprintf(label, sizeof(label), "b_%lu", (uint32_t)i);
        ...
        if (params.transform == TRANSFORM_UR) {
            ...
            snprintf(label, sizeof(label), "G_%lu", (uint32_t)i);
...
{
    return picnic_keygen(Picnic_L3_FS, (picnic_publickey_t *)pk, (picnic_privatekey_t *)sk);
}
...
{
    return picnic_sign((picnic_privatekey_t *)sk, msg, len, sm, (size_t *)smlen);
}
...
{
    return picnic_verify((picnic_publickey_t *)pk, m, *mlen, sm, smlen);
}
